...
class SuperAdaptiveCompositeLoss(nn.Module):
    ...
    def __init__(self, num_classes=3, device='cuda', initial_gamma=2.0, initial_alpha=1.0):
        ...
        self.smooth_weight = nn.Parameter(torch.tensor(0.3, requires_grad=True))  # 权重平滑CE损失
...
def print_loss_status(loss_fn):
    ...
    # --- 新增打印 ---
    focal_w_val = torch.sigmoid(loss_fn.focal_weight).item()
    smooth_w_val = torch.sigmoid(loss_fn.smooth_weight).item()
    print(f"🔍 SACL状态: γ={gamma_val:.3f}, α={alpha_val:.3f}, LS={ls_val:.3f}")
    print(f"🔍 SACL内部权重: focal_w={focal_w_val:.3f}, smooth_w={smooth_w_val:.3f}")
